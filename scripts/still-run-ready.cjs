/** Render arbitrary frames of the card:  node scripts/still-run-ready.cjs 0 3 8 11 20 … */
const fs = require("fs"), path = require("path");
const { bundle } = require("@remotion/bundler");
const { selectComposition, renderStill } = require("@remotion/renderer");
const ROOT = path.join(__dirname, "..");
const { scene, durationInFrames } = require("./build-run-ready-scene.cjs");
const OUT = path.join(ROOT, "data/run-ready/check");
const frames = process.argv.slice(2).map(Number).filter((n) => !Number.isNaN(n));
(async () => {
  fs.mkdirSync(OUT, { recursive: true });
  const dir = path.join(ROOT, "remotion", "scenes");
  const tag = `_still_ready_${Date.now().toString(36)}`;
